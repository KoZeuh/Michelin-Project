...
export class MichelinThreeScene {
  ...
  public init() {
    try {
      ...
      const key = new THREE.DirectionalLight(0xffffff, 2.4); key.position.set(4, 6, 7); this.scene.add(key)
      const rimY = new THREE.DirectionalLight(0xFCE500, 3.4); rimY.position.set(-6, -3, -3); this.scene.add(rimY)
      const rimB = new THREE.DirectionalLight(0x3a78ff, 2.8); rimB.position.set(7, -1, -5); this.scene.add(rimB)
      const fill = new THREE.DirectionalLight(0x6182BB, 0.9); fill.position.set(-5, 4, 3); this.scene.add(fill)
  ...
  private buildWheel() {
    ...
    const rimMat = new THREE.MeshStandardMaterial({ color: 0x0e1119, roughness: 0.34, metalness: 0.55 })
    const aero = new THREE.Mesh(new THREE.TorusGeometry(1.30, 0.30, 12, 200), rimMat); aero.scale.set(1, 1, 0.28); g.add(aero)
    const hook = new THREE.Mesh(new THREE.TorusGeometry(BEAD, 0.05, 16, 200), new THREE.MeshStandardMaterial({ color: 0x3a4254, roughness: 0.3, metalness: 0.8 })); hook.scale.set(1, 1, 0.6); g.add(hook)
    const hub = new THREE.Mesh(new THREE.CylinderGeometry(0.17, 0.17, 0.6, 30), new THREE.MeshStandardMaterial({ color: 0x14181f, roughness: 0.34, metalness: 0.9 })); hub.rotation.x = Math.PI / 2; g.add(hub)
    
    const flMat = new THREE.MeshStandardMaterial({ color: 0x232a36, roughness: 0.4, metalness: 0.85 })
    ;[-0.27, 0.27].forEach(z => { const fl = new THREE.Mesh(new THREE.CylinderGeometry(0.27, 0.27, 0.045, 30), flMat); fl.rotation.x = Math.PI / 2; fl.position.z = z; g.add(fl) })
    
    const casMat = new THREE.MeshStandardMaterial({ color: 0x9aa3b2, roughness: 0.3, metalness: 0.92 })
    for (let k = 0; k < 5; k++) { const c = new THREE.Mesh(new THREE.CylinderGeometry(0.36 - k * 0.035, 0.36 - k * 0.035, 0.028, 28), casMat); c.rotation.x = Math.PI / 2; c.position.z = 0.31 + k * 0.05; g.add(c) }
    
    const spMat = new THREE.MeshStandardMaterial({ color: 0xc2cad6, roughness: 0.3, metalness: 0.85 })
    const NS = 24
    for (let s = 0; s < NS; s++) {
      const a = s / NS * Math.PI * 2
      const z = (s % 2 ? 0.27 : -0.27)
      const p1 = new THREE.Vector3(Math.cos(a) * 0.27, Math.sin(a) * 0.27, z)
      const p2 = new THREE.Vector3(Math.cos(a) * 1.30, Math.sin(a) * 1.30, 0)
  ...
  private buildTread(spec: any) {
    ...
    const w = spec.width
    const Rc = BEAD + w
    const rubber = new THREE.MeshStandardMaterial({ color: 0x0c0f15, roughness: spec.tread === 'slick' ? 0.32 : 0.72, metalness: 0.18 })
    g.add(new THREE.Mesh(new THREE.TorusGeometry(Rc, w, 40, 220), rubber))
    
    if (spec.tread !== 'slick') {
      const cfg = spec.tread === 'cx'
      ...
      const km = new THREE.MeshStandardMaterial({ color: 0x05060a, roughness: 0.86, metalness: 0.1 })
      const im = new THREE.InstancedMesh(new THREE.BoxGeometry(cfg.size[0], cfg.size[1], cfg.size[2]), km, cfg.phis.length * cfg.around)
      ...
      for (let t = 0; t < cfg.around; t++) {
        ...
        cfg.phis.forEach((phi, pi) => {
          ...
          const x = rr * Math.cos(th), y = rr * Math.sin(th), zz = w * Math.sin(phi)
          o.position.set(x, y, zz)
          o.lookAt(x + Math.cos(phi) * Math.cos(th), y + Math.cos(phi) * Math.sin(th), zz + Math.sin(phi))
      ...
      g.add(new THREE.Mesh(new THREE.TorusGeometry(Rc + w * 0.985, 0.01, 8, 200), new THREE.MeshStandardMaterial({ color: 0x000000, roughness: 0.5 })))
    }
    return g
  }
