<script setup lang="ts">
    ...
    const hasAccessibleChildren = (link: NavLink) => link.children?.some((child) => !isChildDisabled(child)) ?? false

    const isGroupDisabled = (link: NavLink) => Boolean(link.children?.length) && !hasAccessibleChildren(link)
                ...
                <div v-if="link.children">
                    ...
                    >
                        ...
                        <button
                            ...
                            "
                            :title="isGroupDisabled(link) ? 'Restricted access' : undefined"
                            @click.stop="!isGroupDisabled(link) && (openGroups[link.label] = !openGroups[link.label])"
                        >
                            <Icon
                                name="ChevronDown"
                    ...
                    <div v-show="openGroups[link.label] && !isGroupDisabled(link)" class="mt-2 space-y-1 pl-8">
                        <template v-for="child in link.children" :key="child.route">
                            ...
                            >
                                <Icon :name="child.icon || 'Dot'" class="h-4 w-4" />
                                <span>{{ child.label }}</span>
                                <span class="ml-auto inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-white/10 text-white/85" aria-hidden="true">
                                    <Icon name="Lock" class="h-3 w-3 text-white" />
                                </span>
                            </button>
                        </template>
